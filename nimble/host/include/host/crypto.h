 ...
 */
int ble_rand(void *buf, size_t len);
 ...
 */
int ble_encrypt_le(const u8_t key[16], const u8_t plaintext[16],
		           u8_t enc_data[16]);
 ...
 *  @return Zero on success or error code otherwise.
 */
int ble_encrypt_be(const u8_t key[16], const u8_t plaintext[16],
		           u8_t enc_data[16]);
 ...
 */
int ble_ccm_decrypt(const u8_t key[16], u8_t nonce[13], const u8_t *enc_data,
 ...
 *  @retval -EINVAL  Invalid parameters.
 */
int ble_ccm_encrypt(const u8_t key[16], u8_t nonce[13], const u8_t *enc_data,
                    size_t len, const u8_t *aad, size_t aad_len,
                    u8_t *plaintext, size_t mic_size);
